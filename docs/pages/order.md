...
10. **Executive Corporate Tax Invoice & Pure Vector PDF Architecture**:
    ...
    - **Genuine Scannable Barcode & Verification Seal (`barkod.studio` Standard)**: High-precision ISO/IEC 15417 Code 128 (Set B) barcode engine (`src/utils/barcode128.js`) modeled on `https://barkod.studio/` specifications. Renders pure vector `<rect>` bars with `shape-rendering="crispEdges"`, standard 10-module quiet zones, and Modulo-103 checksum validation. Decodes order IDs (`ZYP-XXXXXX`) across mobile cameras (iOS Camera, Google Lens), handheld 1D/2D laser scanners, and optical decoders (`@zxing/library`). Embedded as scalable SVG in the web invoice / HTML download and as pure vector rectangles in the PDF export.
    - **Vector PDF Engine (`jspdf` & `src/utils/generateInvoicePdf.js`)**: 100% client-side vector generation without canvas or CORS overhead, downloading in <50ms across all devices with native vector barcode rendering.
