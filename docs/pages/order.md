...
9. **Instant Digital Invoice & PDF Generation**:
   ...
   - **Client-Side Vector PDF Export (`html2pdf.js`)**: Converts the styled computerized invoice into a high-fidelity A4 PDF document (`Zyphuel-Invoice-${id}.pdf`) directly on the client with 2x scale crisp rendering and graceful fallback to browser print.
...
| :--- | :--- | :--- |
| **2026-09-21** | Revamped post-order flow: replaced premature instant WhatsApp pop-up with official Invoice & PDF download view; integrated `html2pdf.js` for instant client-side `.pdf` invoice generation; added 3-second animated auto-redirect countdown to WhatsApp Dispatch (`+92 3230-112464`) with instant manual trigger and cancellation controls. | User requested: *"order place hone kab bad invoice pahle pdf ma download option kare or phir WhatsApp per redirect karo"*. Upon order submission, the official invoice modal opens immediately displaying a prominent post-order hero banner with "1. Download Invoice (PDF)" and "2. Proceed to WhatsApp Dispatch". Clicking download triggers client-side generation of `Zyphuel-Invoice-${id}.pdf` and initiates a 3s countdown transition to the prefilled WhatsApp dispatch helpline. Also added direct WhatsApp dispatch action buttons in modal header and footer, plus updated tracker modal button. |
