...
export async function generateInvoicePdf(data) {
  ...
  // 7. Official Digital Verification Stamp & Legal Signatory Block
  const stampY = summaryY + 41
  
  // Left: Official Seal Representation
  doc.setDrawColor(2, 132, 199)
  doc.setLineWidth(0.6)
  doc.roundedRect(margin + 5, stampY, 68, 16, 1.5, 1.5)
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(6.5)
  doc.setTextColor(2, 132, 199)
  doc.text('★ ZYPHUEL PAKISTAN • CERTIFIED DISPATCH ★', margin + 9, stampY + 5)
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(6)
  doc.setTextColor(100, 116, 139)
  doc.text('Automated Depots Dispatch Gateway • Lahore Hub #01', margin + 9, stampY + 9)
  doc.text('Digital Timestamp: ' + (data.date || 'Verified Session'), margin + 9, stampY + 13)

  // Right: Computerized Signatory Notice
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(7.5)
  doc.setTextColor(15, 23, 42)
  doc.text('Computerized Verified Commercial Invoice', pageWidth - margin - 5, stampY + 5, { align: 'right' })
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(6.8)
  doc.setTextColor(100, 116, 139)
  doc.text('Generated via Zyphuel Fleet Automation & Telemetry Gateway', pageWidth - margin - 5, stampY + 9.5, { align: 'right' })
  doc.text('No physical signature required (Electronic Data Interchange Standard)', pageWidth - margin - 5, stampY + 13.5, { align: 'right' })
